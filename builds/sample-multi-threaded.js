...
function findWasmBinary() {
  var f = "sample-multi-threaded.wasm";
  if (!isDataURI(f)) {
    return locateFile(f);
  }
  return f;
...
var PThread = {
  ...
  tlsInitFunctions: [],
  pthreads: {},
  pthreadShutdownStates: {},
  mainThreadMailboxShutdown: {},
  init() {
    addOnInit(() => {
...
var __emscripten_thread_mailbox_await = pthread_ptr => {
  if (typeof Atomics.waitAsync === "function") {
    ...
    if ((!(ENVIRONMENT_IS_PTHREAD))) {
      Promise.race([ wait.value, PThread.mainThreadMailboxShutdown ]).then(checkMailbox);
    } else {
...
var checkMailbox = () => {
  ...
  if ((!(ENVIRONMENT_IS_PTHREAD)) && PThread.mainThreadMailboxShutdown.resolved) {
    return;
  }
...
isPthread && HeavyObjectDemoModuleMT();
